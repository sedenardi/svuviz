...
module.exports = function(db) {
  return {
    ...
    colorCutoffs: function(baseTitleId) {
      var sql = `
      create table cActors ( ActorID varchar(10) );
      create index ix on cActors (ActorID);
      insert into cActors
      select ActorID
      from Appearances a
      where exists
          ...
          left outer join
              ...
              from Appearances app1
                ...
                inner join Appearances app2
                  ...
                  and app2.ActorID <> app1.ActorID
          where exists
                (select 1 from cActors ra1
                where ra1.ActorID = app1.ActorID)
          and exists
                (select 1 from cActors ra2
                where ra2.ActorID = app2.ActorID)) c
            on c.ActorID1 = a1.ActorID
        group by a1.ActorID,a1.Name) c
      ...
      return db.query([sql, [baseTitleId, baseTitleId]]).then((res) => {
        var cutoffs = [res[8][0].first, res[8][0].second, res[8][0].third];
        return Promise.resolve(cutoffs);
      });
    },
    baseTitleInfo: function(baseTitleId) {
      return Promise.all([
        this.showInfo(baseTitleId),
        this.filterTitles(baseTitleId),
        this.colorCutoffs(baseTitleId)
      ]).then((res) => {
